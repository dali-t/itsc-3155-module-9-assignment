...
def test_search_with_valid_entry(test_app):
    response = test_app.get("/movies/search?movie-search=The+Dark+Knight")
...
def test_search_case_insensitivity(test_app):
    response = test_app.get("/movies/search?movie-search=tHe+DaRk+KnIgHt")
...
def test_search_with_invalid_entry(test_app):
    response = test_app.get("/movies/search?movie-search=Kung+Fu+Panda")
    data = response.data.decode('utf-8')
